...
Subscribes to the current price of a pair in a particular direction. This is useful for liquidity providers to see the current price of a pool. Note `{"base_asset": "BTC", "quote_asset": "USDC }` is not equivalent to `{"base_asset": "USDC", "quote_asset": "BTC"}, `, therefore the prices maybe different.
...
It returns the price at every block, even if there are no changes.
